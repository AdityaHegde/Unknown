...
function handleRequest(req, res, params, method, methodAlternate) {
  ...
  methodRef = mongodb_data_adaptor.hasModel(model) ? mongodb_data_adaptor[methodToUse] : file_data[methodToUse],
  meta = {};
  methodRef(model, query, function(err, data) {
    if(err) {
